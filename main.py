...
#with app.app_context():
    #db.create_all()    #db is already created so i am gonna put this line in a comment



video_put_args = reqparse.RequestParser()
video_put_args.add_argument("name",type=str,help="Name of the video",required=False)
...
def abort_if_video_exists(video_id):
    if video_id in videos:
        abort(404, message="Video is already in videos...")
...
class Video(Resource):
    @marshal_with(resource_fields) #ovo uzima objekat koji returnujemo u pretvara ga u resource fields, da bismo mogli da ga procitamo
    ...
    def put(self,video_id):
        ...
        result=db.session.query(VideoModel).filter_by(id=video_id).first()
        if result:
